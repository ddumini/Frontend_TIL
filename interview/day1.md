...
**Git Flow는 브랜치 관리 전략중 하나로, 명확한 브랜치 역할 분담을 통해 대규모 프로젝트의 버전 관리를 체계적으로 할 수 있는 모델입니다.**

**Git Flow의 5가지 주요 브랜치**

### **1. main (master) 브랜치**

- **역할**: 프로덕션에 배포 가능한 안정적인 코드만 관리
- **특징**: 모든 커밋이 릴리즈 버전을 나타내며, 태그로 버전 관리
- **병합**: release 브랜치나 hotfix 브랜치에서만 병합

### **2. develop 브랜치**

- **역할**: 개발의 중심이 되는 브랜치로, 다음 릴리즈를 위한 기능들이 통합
- **특징**: 항상 최신 개발 상태를 유지하며, feature 브랜치들이 병합되는 곳
- **병합**: feature 브랜치에서 완성된 기능들이 병합

### **3. feature 브랜치**

- **역할**: 새로운 기능 개발을 위한 브랜치
- **명명**: feature/기능명 (예: feature/user-authentication)
- **생성**: develop 브랜치에서 분기
- **병합**: 기능 완성 후 develop 브랜치로 병합 후 삭제

### **4. release 브랜치**

- **역할**: 릴리즈 준비를 위한 브랜치
- **명명**: release/버전명 (예: release/1.2.0)
- **특징**: 버그 수정과 메타데이터 준비만 허용, 새로운 기능 추가는 금지
...
- 명확한 브랜치 역할 분담으로 협업이 체계적
- 릴리즈 관리가 안정적이고 예측 가능
- 대규모 팀에서 충돌을 최소화할 수 있음

**단점:**

- 브랜치가 많아 복잡할 수 있음
- 지속적 배포(CD)에는 적합하지 않음
- 작은 프로젝트에는 과도할 수 있음